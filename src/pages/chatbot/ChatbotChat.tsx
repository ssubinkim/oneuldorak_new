...
type ChatRouteContext = {
  ...
  photoDraft: ChatbotPhotoDraft | null
  openPicker: boolean
...
function readChatbotPhotoDraftFromSession(): ChatbotPhotoDraft | null {
  if (typeof window === 'undefined') {
    return null
  }

  try {
    const rawDraft = window.sessionStorage.getItem(CHATBOT_PHOTO_DRAFT_STORAGE_KEY)
    if (!rawDraft) {
      return null
    }
    ...
    const draft = parsed as {
      ...
      displayText?: unknown
      purposeFeature?: unknown
    }
    const purposeFeature = readPhotoPurposeFeature(
      typeof draft.purposeFeature === 'string' ? draft.purposeFeature : null,
    )

    if (
      typeof draft.imageDataUrl === 'string'
      && draft.imageDataUrl.startsWith('data:image/')
      && typeof draft.displayText === 'string'
      && purposeFeature
    ) {
      return {
        imageDataUrl: draft.imageDataUrl,
        displayText: draft.displayText.trim() || '사진 첨부했어. 분석해줘.',
        purposeFeature,
      }
    }
  } catch {
    ...
    try {
      window.sessionStorage.removeItem(CHATBOT_PHOTO_DRAFT_STORAGE_KEY)
    } catch {
...
function getRouteContext(): ChatRouteContext {
  ...
  const shouldReadPhotoDraft = params.get('photoDraft') === '1'
  const judgeMode =
    params.get('judge') === '1'
      ? routeMode === 'photo'
        ? 'photo'
  ...
  return {
    ...
    photoDraft: shouldReadPhotoDraft ? readChatbotPhotoDraftFromSession() : null,
...
function ChatbotChat() {
  ...
  const handleDemoPhotoPurposeSelect = useCallback(async (purposeFeature: PhotoPurposeFeature) => {
    ...
    try {
      ...
      submitPhotoForAnalysis(imageDataUrl, displayText, purposeFeature, { useApi: false })
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (context.photoDraft) {
      const { imageDataUrl, displayText, purposeFeature } = context.photoDraft
      setSelectedPhotoPurpose(purposeFeature)
      activeFeatureRef.current = purposeFeature
      activeAnalysisTypeRef.current = getAnalysisTypeForFeature(purposeFeature)
  ...
  }, [handleDemoPhotoPurposeSelect, openCameraSheet, openDesktopCamera, queueUserRequest, submitPhotoForAnalysis])
